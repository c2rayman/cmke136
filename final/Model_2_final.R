...
model2_main <- function(){
  ...
  #create new data frame with only ngrams of 3
  newData <- createNewDataFrame(fileData,gramLength)
  
  #create data frame of terms freqeuncy so logistic regression can be performed
  newData2 <- createTermMartix(newData)
  formula = as.formula('out_put_class ~ .')
  weka_fit2 <- Logistic(formula, data = newData2)
  print(evaluate_Weka_classifier(weka_fit2, numFolds = 10))
  
  print("Ending")
}

#Function to create new data frame with reverse transcriptase split into length
#of ngrams of 3
...
#function to create a term martix 2
createTermMartix <- function(dataIn){
  ...
  #frequeny words
  print(sort(as.matrix(dtm_tfidf)[1,], decreasing=T)[1:5])
  
  #convert into data frame
  df1 <- as.data.frame(as.matrix(dtm_tfidf))
  dataOut <- cbind(df1,out_put_class=as.factor(dataIn$Resp))
  return(dataOut)
}
